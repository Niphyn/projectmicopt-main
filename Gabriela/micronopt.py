...
class Interrogator(object):
    def __init__(self, ip_address="10.0.0.122", port=50000, fbg_props=None): # ip_address="192.168.1.166", port=1852
    ...
    def send_command(self, command, receive=True):
        ...
        if receive:
            ...
            print(respsize) # The response structure is at most 128108 (20+4*20+4*2*16001) bytes long for a four-channel unit.
    ...
    def data_debug(self):
        ...
        return response
        #return header, data
        #return self.latest_response.decode("ascii")
        #return self.latest_response.decode(encoding='utf_32')
        #return self.latest_response.decode() # ?
    ...
    def debug_append_data(self):
        self.create_debug_data_dict()
        self.append_data = True
        
    def create_debug_data_dict(self):
        self.data.clear()
        sensor_name = "sensor"
        self.data[sensor_name + "_wavelength"] = np.array([])
        self.data[sensor_name + "_measurement"] = np.array([])
        self.data["timestamp"] = np.array([])
        self.data["time"] = np.array([])
        self.data["serial_no"] = np.array([])
    
    def debug_append_data(self): # incompleto/errado
        ...
        self.data[sensor_name + "_wavelength"] = np.append(self.data[s.name + "_wavelength"],s.wavelength)
        #self.data[sensor_name + "measurement"] = np.append(self.data[
